...
def _validate_generation_args(n_samples, size, n_classes, available_classes):
    """Validate public generator arguments before allocating any tensors."""
    if not isinstance(n_samples, int) or isinstance(n_samples, bool) or n_samples < 1:
        raise ValueError("n_samples must be a positive integer.")
    if not isinstance(size, int) or isinstance(size, bool) or size not in VALID_SIZES:
        raise ValueError(f"size must be one of {VALID_SIZES}; received {size!r}.")
    if not isinstance(n_classes, int) or isinstance(n_classes, bool):
        raise ValueError("n_classes must be an integer.")
    if not 1 <= n_classes <= len(available_classes):
        raise ValueError(
            f"n_classes must be between 1 and {len(available_classes)}; "
            f"received {n_classes!r}."
        )
...
def generate_2d_shapes(n_samples, size=64, n_classes=4, seed=None):
    ...
    """
    _validate_generation_args(n_samples, size, n_classes, SHAPE_CLASSES_2D)
...
def generate_3d_shapes(n_samples, size=64, n_classes=4, seed=None):
    ...
    """
    _validate_generation_args(n_samples, size, n_classes, SHAPE_CLASSES_3D)
    generator = torch.Generator().manual_seed(seed) if seed is not None else None
